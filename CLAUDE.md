...
A static family website for Matt and Sarah Shaeffer (Deatsville, Alabama). No build system, no dependencies, no package manager — plain HTML, CSS, and vanilla JS served directly from files.
...
**Interior pages** (`projects.html`, `curriculum.html`) use a separate shared design system:
- `styles.css` — full design token system (CSS custom properties at `:root`), plus all component styles for nav, bento grids, cards, footer, and mobile bottom nav
- `main.js` — small IIFE that marks active nav links on page load
...
Primary palette defined in `styles.css`:
...
Edit the `data` array in the inline `<script>` at the bottom of `index.html`. Each entry needs `section`, `title`, `title2`, `description`, and `image` fields. The carousel and `order` array are built from `data.length` automatically.

## Image inventory

All images live in `images/`. Currently: `mattandsarah.jpg`, `church.jpg`, `tree-kids.jpg`, `cow.jpg` — all used by the homepage carousel.

## Active nav highlighting

`main.js` handles active state for `.nav-links a`, `.footer-nav-col a`, and `.bottom-nav-item` by comparing `href` to `window.location.pathname`. Every interior page must include `<script src="main.js"></script>` before `</body>`.
